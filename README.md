...
Version 0.4 (light):
1) Added ability to change "sigma" parameter (important for segmentation, higher sigma will segment large bacteria more correctly).
2) Reorganized GUI. Removed some parts, that is currently not used.

Version 0.3.1 (light):
1) Switcher between original and segmented image

Version 0.3 (light):
1) Following my research on bacteriological segmentation, "Bacteria-finder_light" version is currently developed. It features lighter and faster segmentation algorithm (not based on neural network, but on traditional approaches). In the future development I am planning to add light classification as well.
